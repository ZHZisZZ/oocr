...
for learning_rate in 1e-6 3e-6 1e-5 3e-5; do
    (
        output_base_dir=${base_dir}/${data_config}/$(basename ${model_name_or_path})/lr-${learning_rate}
        for seed in $(seq 0 5); do
            WANDB_MODE=disabled WANDB_PROJECT=oocr PYTHONPATH=. srun -p mllm_safety --quotatype=reserved --gres=gpu:${gpu} --cpus-per-task=16 --time=3000 \
